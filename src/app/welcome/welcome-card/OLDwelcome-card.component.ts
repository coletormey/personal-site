...
export class WelcomeCardComponent implements AfterViewInit {
  ...
  private projectsOptionMesh: THREE.Mesh;
  private servicesOptionMesh: THREE.Mesh;
  ...
  private createMenuObjects() {
    ...
    const projectsOptionMeshGeometry = new THREE.BoxGeometry(1, 1, 1);
    const projectsOptionMeshMaterial = new THREE.MeshBasicMaterial({ map: this.loader.load(this.texture) });
    this.projectsOptionMesh = new THREE.Mesh(projectsOptionMeshGeometry, projectsOptionMeshMaterial);

    const servicesOptionMeshGeometry = new THREE.BoxGeometry(1, 1, 1);
    const servicesOptionMeshMaterial = new THREE.MeshBasicMaterial({ map: this.loader.load(this.texture) });
    this.servicesOptionMesh = new THREE.Mesh(servicesOptionMeshGeometry, servicesOptionMeshMaterial);
...
/*    this.menuOptions = new THREE.Group();
    ...
    this.flow_profileOption.object3D.userData = { currentMenuPosition: 0 };
    ...
    this.flow_systemOption.object3D.userData = { currentMenuPosition: 1 };

    this.flow_servicesOption = new Flow(this.servicesOptionMesh);
    ...
    this.flow_servicesOption.object3D.userData = { currentMenuPosition: 2 };

    this.flow_projectsOption = new Flow(this.projectsOptionMesh);
    ...
    this.flow_projectsOption.object3D.userData = { currentMenuPosition: 3 };
    console.log(this.flow_projectsOption.object3D.scale.toArray.toString);
    ...
    if (intersects.length > 0 && intersects[0].object !== this.line ) {
      ...
      switch (this.INTERSECTED) {
        case this.leftArrow:
          ...
          temp = this.array_menuOptions[0]
          for (let i = 0; i < this.array_menuOptions.length - 1; i++) {
            if (i < this.array_menuOptions.length - 1) {
              this.array_menuOptions[i] = this.array_menuOptions[i + 1];
            } else {
              this.array_menuOptions[i] = temp;
            }
          }
